...
- Added Copy Screenshot in the render-surface context menu. It writes a temporary PNG through libmpv, copies it to the macOS pasteboard, and removes the temporary file.
- Added core coverage confirming screenshot-to-file forwards the requested destination to the playback engine.
- Added Fit to Video in the compact Video menu. It uses libmpv display dimensions captured when media becomes ready, preserves the video ratio (including Nura rotation), and constrains the window to the active screen's visible frame.
...
- Rust tests, formatting checks, Xcode Debug build, Release packaging, and `git diff --check` passed.
- Manual media workflow verification remains limited by the headless automation context; PiP/OpenGL, Fit to Video across displays, screenshot clipboard, and public site playback should receive an interactive smoke test before shipping.
