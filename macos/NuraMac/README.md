...
Release builds and Archives run the `Bundle Verified libmpv Runtime` build phase. It copies the lock-verified `libmpv` dependency closure and FFmpeg audio tools into `Nura.app`, so the resulting app does not require Homebrew on the user's Mac. Run `../../scripts/test-libmpv-runtime.sh` from this directory to verify the runtime independently.
